...
source "[list [file join [file dirname [info script]] sdram_phy_ddr_timing.tcl]]"
...
# instantiations.
################################################################################
proc add_requirements_for_instance {corename instance_name t_name board_name ISI_name sdram_phy_use_flexible_timing} {
	upvar 1 $t_name t
...
################################################################################
	foreach dqsgroup $pins(dqsgroup) {
		...
		if {$sdram_phy_use_flexible_timing} {
			# DQS output clock
			...
			if {$sdram_phy_use_flexible_timing && ($::quartus(nameofexecutable) ne "quartus_fit")} {
				lappend ::dqs_clocks $dqs_out_clockname
			}
		}
		# endif $sdram_phy_use_flexible_timing
		if {$sdram_phy_use_flexible_timing} {
...
set_false_path -to [get_pins -compatibility_mode ${instname}_alt_mem_phy_inst|dpio|dqs_group\[*\].dq\[*\].dqi|auto_generated|input_*_*\[0\]|clrn]
	...
	if {($::quartus(nameofexecutable) == "quartus_fit") || !$sdram_phy_use_flexible_timing} {
...
################################################################################
	...
	if {$sdram_phy_use_flexible_timing && (($::quartus(nameofexecutable) ne "quartus_fit") && ($::quartus(nameofexecutable) ne "quartus_map"))} {
...
foreach inst $instance_list {
	...
	add_requirements_for_instance $corename $inst t board ISI $sdram_phy_use_flexible_timing
